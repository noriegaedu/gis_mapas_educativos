...
etr <- st_read("H:/mmaya/sig/EMPAQUETADOS BHSB/etr/commondata/downloads/WB_Historico_CC_CCL.shp") %>% 
    st_transform(4326) %>% 
    mutate(categ = cut(etr$Balance__4, c(0,200,500,700,900,1000,1200), 
                       include.lowest = TRUE, dig.lab = 10, 
                       labels = c('< 200', '200 - 500', '500 - 700', '700 - 900', '900 - 1000', '> 1000'))) 
levels(etr$categ) <- c(levels(etr$categ),'Sin Resultados')
etr$categ[etr$Balance__4 == 0] <- as.factor('Sin Resultados')
...
etrg <- ggplot() +
    ...
    geom_sf(data = etr, aes(fill = categ), color = NA) +
    ...
    scale_fill_manual(values = c('Sin Resultados' = 'lightpink',
                                 '< 200' = RColorBrewer::brewer.pal(9,'YlOrRd')[2],
                                 '200 - 500' = RColorBrewer::brewer.pal(9,'YlOrRd')[3],
                                 '500 - 700' = RColorBrewer::brewer.pal(9,'YlOrRd')[4],
                                 '700 - 900' = RColorBrewer::brewer.pal(9,'YlOrRd')[5],
                                 '900 - 1000' = RColorBrewer::brewer.pal(9,'YlOrRd')[7],
                                 '> 1000' = RColorBrewer::brewer.pal(9,'YlOrRd')[9]),
...
ggsave('H:/mmaya/sig/EMPAQUETADOS BHSB/sencillos/etr.jpg', etr_ed, units = c('cm'),
