...
precip <- st_read("H:/mmaya/sig/EMPAQUETADOS BHSB/precipitacion/commondata/downloads1/WB_Historico_CC_CCL.shp") %>% 
    st_transform(4326) %>% 
    mutate(categ = cut(precip$Balance__3, c(0,200,seq(500,1500,500),2500,6000), 
                       include.lowest = TRUE, dig.lab = 10, 
                       labels = c('< 200', '200 - 500', '500 - 1000', '1000 - 1500', '1500 - 2500', '> 2500'))) 
levels(precip$categ) <- c(levels(precip$categ),'Sin Resultados')
precip$categ[precip$Balance__3 == 0] <- as.factor('Sin Resultados')
...
prec <- ggplot() +
    ...
    geom_sf(data = precip, aes(fill = categ), color = NA) +
    ...
    scale_fill_manual(values = c('Sin Resultados' = 'lightpink',
                                 '< 200' = RColorBrewer::brewer.pal(9,'YlGnBu')[2],
                                 '200 - 500' = RColorBrewer::brewer.pal(9,'YlGnBu')[3],
                                 '500 - 1000' = RColorBrewer::brewer.pal(9,'YlGnBu')[4],
                                 '1000 - 1500' = RColorBrewer::brewer.pal(9,'YlGnBu')[5],
                                 '1500 - 2500' = RColorBrewer::brewer.pal(9,'YlGnBu')[7],
                                 '> 2500' = RColorBrewer::brewer.pal(9,'YlGnBu')[9]),
                      name = 'Precipitación \n[mm/año]',
...
ggsave('H:/mmaya/sig/EMPAQUETADOS BHSB/sencillos/prec.jpg', prec_ed, units = c('cm'),
